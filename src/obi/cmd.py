...
ROBOT = [
    "java", "-jar", "build/robot.jar", "--prefix",
...
def prepare_robot_module(ontology):
    ...
    args = [
        "merge",
        "--input", source_path,
        "template", "--template", parent_tsv_path,
        "annotate", "--ontology-iri", iri,
        "--output", parent_owl_path
        ]
    command = ROBOT + args
    run(command)
...
def build_robot_module(ontology):
    ...
    module = os.path.join("src", "ontology",
                          "robot_outputs", f"{ontology}_imports.owl")
    ...
    mireot_args = [
        "extract", "--method", "MIREOT", "--input", source,
        "--upper-terms", limit, "--lower-terms", term,
        "--intermediates", "minimal", "--annotate-with-source", "true",
        "export", "--header", "IRI", "--export", mireot
        ]
    subset_args = [
        "extract", "--method", "subset", "--input", source,
        "--term-file", mireot, "--term-file", relation,
        "--annotate-with-source", "true"
            ]
    remove_args = [
        "remove", "--term-file", ignore,
            ]
    finalize_args = [
        "reduce", "--reasoner", "ELK",
        "merge", "--input", parent,
...
def build_ontofox_module(ontology):
    ontology = ontology.upper()
    config = os.path.join("src", "ontology",
                          "OntoFox_inputs", f"{ontology}_input.txt")
    ...
    args = [
        "curl", "-s", "-F", f"file=@{config}", "-o", build_module,
        "https://ontofox.hegroup.org/service.php"
    ]
    run(args)
    if ontology == "CLO":
        args2 = [
            "remove", "--input", build_module,
            "--select", "annotation-properties", "--trim", "false",
            "--output", output
        ]
    else:
        args2 = [
            "convert", "-i", build_module, "-o", output
        ]
    command = ROBOT + args2
    run(command)
